    ...
    methods: {
      eAtivo(menu) {
        if (this.$route.params.visao !== undefined) {
          return this.$route.params.visao.toLowerCase() === menu.rotulo.toLowerCase();
        }
      }
    }
  }
...
<style lang="scss" scoped>
  ...
  /deep/.q-expansion-item--expanded.submenu {
    .q-expansion-item__container .q-item.q-item-type.row.no-wrap.q-item--clickable.q-link.cursor-pointer.q-focusable.q-hoverable {
      background: rgba(0, 0, 0, 0.1) !important;
    }
  }

  .submenu_ativo {
    background-image: linear-gradient(to bottom, #037fff, #0084ff, #0088fe, #008dfe, #0091fd, #0394fc, #0a97fc, #129afb, #149dfb, #179ffa, #1ba2fa, #20a4f9) !important;
    color: #fff;
    font-weight: 600;
  }
</style>
